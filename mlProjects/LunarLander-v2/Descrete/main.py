...
class program:
    ...
    def __init__(self, gamma, epsilon, learningRate, graphical):
        ...
        self.qTable = np.zeros((200,4))
        self.rewardTable = np.zeros((500))
    ...
    def updateQTable(self):
        ...
        reward = self.reward
        oldQ = self.qTable2[lastState][action][0]
        nextState = self.qTable2[lastState][action][1]

        nextStateQ = 0
        if not nextState == -1:
            nextStateQ = self.qTable2[nextState][self.chooseAction(nextState, 0)][0]
        else:
            nextStateQ = 0

        lr, gamma = self.learningRate, self.gamma

        qVal = oldQ +  lr*( reward + gamma*nextStateQ - oldQ )
